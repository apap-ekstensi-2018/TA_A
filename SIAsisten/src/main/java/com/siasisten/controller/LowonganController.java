...
public class LowonganController {
	...
	/*Opsi 2*/
	/*@RequestMapping("/lowongan/tambah")
	public String tambahLowongan(Model model, @RequestParam(value = "id_matkul", required = false) int id_matkul,
											  @RequestParam(value = "is_open", required = false) boolean is_open,
											  @RequestParam(value = "jml_lowongan", required = false) int jml_lowongan)
	{
		List<MatkulModel> matkul = matkulDao.selectAllMatkul();
		model.addAttribute("matkul", matkul);
		if(lowongan.getIdMatkul()==0) {
